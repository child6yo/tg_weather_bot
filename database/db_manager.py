...
async def change_user_chat_style(user_id: int, chat_style: str, table_name="tg_users"):
    """Изменяет стиль чата пользователя в базе данных."""
    async with db_manager as client:
        await client.update_data(
            table_name=table_name,
            where_dict={"user_id": user_id},
            update_dict={"chat_style": chat_style},
        )